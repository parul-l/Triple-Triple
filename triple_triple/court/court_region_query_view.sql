/*
Because we hope to modify/grainfy the court regions, instead of making a table,
we use a view with gameid and playerids as variables.
When court_regions are needed, the view is created, joined with appropriate table, 
and then dropped. Example: player_possession_habits.py
*/
        ...
        , CASE WHEN (shooting_side = 'left' 
            ...
            WHEN (shooting_side = 'left'
                AND (x_coordinate >= 19 AND x_coordinate <= 25 AND
                    POWER(x_coordinate - 19, 2) + POWER(x_coordinate - 25, 2) <= POWER(6, 2)))
                OR  (shooting_side = 'right'
                AND (x_coordinate >= 69 AND x_coordinate <= 75 AND
                    POWER(x_coordinate - 75, 2) + POWER(x_coordinate - 25, 2) <= POWER(6, 2)))
                THEN 'key'
            WHEN (shooting_side = 'left'
