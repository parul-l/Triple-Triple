...
def player_court_region_df(df_pos_dist):
    ...
    for j in range(len(player_list)):
        ...
        for i in range(len(df_pos_x_loc)):
            ...
            if math.isnan(df_pos_x_loc[i,j]) != True:
                player_court_region[i] = region(df_pos_x_loc[i,j],\
                                            df_pos_y_loc[i,j],\
                                            shooting_side)
...
def characterize_player_possessions(\
    ...
    for j in range(len(player_ball_idx)):
        ...
        shooting_side = team_shooting_side(player_name, period_play_start, initial_shooting_side, hometeam_id, awayteam_id) 

        start_region = region(df_pos_dist_trunc[player_name].x_loc.iloc[play_start_index],df_pos_dist_trunc[player_name].y_loc.iloc[play_start_index], shooting_side)

        # End of play      
        play_end_index = next_player_ball_idx[j]-1
        game_clock_play_end = df_pos_dist_trunc.game_clock.values.flatten()[play_end_index]
        
        end_region = region(df_pos_dist_trunc[player_name].x_loc.iloc[play_end_index],df_pos_dist_trunc[player_name].y_loc.iloc[play_end_index], shooting_side)
...
def pass_not_assist(player_name, df_pos_dist_trunc, known_player_possessions, player_poss_idx, t=10):
    ...
    for i in range(len(start_idx_not_used)):
        ...
        if next_teammate != player_name:
            ...
            if len(set(closest_player_to_ball[next_team_idx:next_team_idx+t]))==1:
                ...
                start_region = region(df_pos_dist_trunc[player_name].x_loc.iloc[play_start_index], df_pos_dist_trunc[player].y_loc.iloc[play_start_index], shooting_side)

                # end of play
                # index where ball ends up      
                game_clock_play_end = df_pos_dist_trunc.game_clock.values.flatten()[next_team_idx]

                end_region = region(df_pos_dist_trunc[next_teammate].x_loc.iloc[next_team_idx],df_pos_dist_trunc[next_teammate].y_loc.iloc[next_team_idx], shooting_side)
