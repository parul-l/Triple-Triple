...
from triple_triple.prob_player_possessions import (
    get_reg_to_num,
    get_action_to_num,
    get_prob_count_matrix(count_matrix)
)
...
def get_df_possession_defender(
    ...
    for player_class in players_dict.values():
        player_id_list = [x.player_id for x in players_dict.values()]
...
def poss_outcome_when_defending(
    defender_id,
    ...
    query_params = 'defender_id==@defender_id and possession_end'
    action_dict = Counter(
        df_possession_defender
        .query(query_params)
        .action
    ...
    return action_array / np.sum(action_array)


def poss_outcome_when_defending_reg(
    defender_id,
    df_possession_defender,
    game_id=None
):
    ...
    return get_prob_count_matrix(action_matrix)


# where defender is vs. player with ball
def get_court_region_matrix(defender_id, df_possession_defender, game_id=None):
    if game_id is not None:
        df_possession_defender = df_possession_defender.query('game_id==@game_id')

    df_player = df_possession_defender\
        .query('defender_id==@defender_id')
    ...
    return get_prob_count_matrix(defending_region_matrix)
